...
let GoogleService = GoogleService_1 = class GoogleService {
    constructor(configService) {
        ...
        this.credentialsPath = path.join(process.cwd(), this.configService.get('GOOGLE_CREDENTIALS_PATH') || 'google-credentials.json');
        this.scopesAPI = this.configService.get('GOOGLE_SCOPES_API').split(',');
    }
    onModuleInit() {
        if (!fs.existsSync(this.credentialsPath)) {
            this.logger.error(`Google credentials file not found at path: ${this.credentialsPath}`);
            throw new Error(`Google credentials file not found at path: ${this.credentialsPath}`);
        }
    }
    ...
    async getOAuth2ClientUrl() {
        const authClient = this.getAuthClient();
        return this.getAuthUrl(authClient);
    }
    getAuthClient() {
        const keys = this.readCredentials(this.credentialsPath);
        const authClient = new google_auth_library_1.OAuth2Client(keys.web.client_id, keys.web.client_secret, keys.web.redirect_uris[0]);
        return authClient;
    }
    getAuthUrl(authClient) {
    ...
    async getAuthClientData(code) {
        try {
            ...
            throw new common_1.BadRequestException('Failed to get auth client data');
        }
    }
};
